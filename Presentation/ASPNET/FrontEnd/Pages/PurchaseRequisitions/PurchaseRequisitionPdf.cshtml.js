const App = {
    setup() {
        ...
        const services = {
            getPDFData: async (id) => {
                try {
                    const response = await AxiosManager.get('/PurchaseRequisition/GetPurchaseRequisitionSingle?id=' + id, {});
                    return response;
                } catch (error) {
                    throw error;
                }
        ...
        const methods = {
            populatePDFData: async (id) => {
                ...
                state.items = pdfData.purchaseRequisitionItemList || [];
                state.vendor = pdfData.vendor || {};
                state.requisitionNumber = pdfData.number || '';
                state.requisitionDate = DateFormatManager.formatToLocale(pdfData.requisitionDate) || '';
                state.requisitionCurrency = StorageManager.getCompany()?.currency || '';
            ...
            downloadPDF: async () => {
                ...
                try {
                    ...
                    await html2canvas(content, {
                        ...
                        doc.save(`purchase-requisition-${state.requisitionNumber || 'unknown'}.pdf`);
                    });
                } catch (error) {
