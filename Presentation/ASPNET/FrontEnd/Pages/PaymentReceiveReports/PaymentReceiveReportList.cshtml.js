const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => {
                try {
                    const response = await AxiosManager.get('/PaymentReceive/GetPaymentReceiveReport', {});
                    return response;
                } catch (error) {
                    throw error;
                }
        ...
        const mainGrid = {
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    beforeDataBound: () => { },
                    dataBound: function () {
                        mainGrid.obj.autoFitColumns(['customerName' ,'number', 'paymentDate', 'invoiceNumber', 'debitNoteNumber', 'paymentMethodName', 'paymentAmount', 'statusName', 'createdAtUtc']);
                    },
                    excelExportComplete: () => { },
                    rowSelected: () => { },
                    rowDeselected: () => { },
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['PaymentReceiveReports']);
