    ...
    setup() {
        ...
        const services = {
            ...
            createMainData: async (name, description, createdById) => {
                try {
                    const response = await AxiosManager.post('/PaymentMethod/CreatePaymentMethod', {
            ...
            updateMainData: async (id, name, description, updatedById) => {
                try {
                    const response = await AxiosManager.post('/PaymentMethod/UpdatePaymentMethod', {
            ...
            deleteMainData: async (id, deletedById) => {
                try {
                    const response = await AxiosManager.post('/PaymentMethod/DeletePaymentMethod', {
                        id, deletedById
                    });
                    return response;
                } catch (error) {
                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    toolbarClick: (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Add Payment Method';
                            state.id = '';
                            state.name = '';
                            state.description = '';
                            mainModal.obj.show();
                        ...
                        if (args.item.id === 'EditCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                ...
                                state.mainTitle = 'Edit Payment Method';
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Delete Payment Method?';
                                state.id = selectedRecord.id ?? '';
                                state.name = selectedRecord.name ?? '';
                                state.description = selectedRecord.description ?? '';
                                mainModal.obj.show();
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['PaymentMethods']);
