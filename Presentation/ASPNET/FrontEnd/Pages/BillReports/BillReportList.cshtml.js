const App = {
    setup() {
        ...
        const services = {
            getMainData: async () => {
                try {
                    const response = await AxiosManager.get('/Bill/GetBillReport', {});
                    return response;
                } catch (error) {
                    throw error;
                }
        ...
        const mainGrid = {
            ...
            create: async (dataSource) => {
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    beforeDataBound: () => { },
                    dataBound: function () {
                        mainGrid.obj.autoFitColumns(['vendorName' ,'number', 'billDate', 'purchaseOrderNumber', 'afterTaxAmount', 'statusName', 'createdAtUtc']);
                    },
                    excelExportComplete: () => { },
                    rowSelected: () => { },
                    rowDeselected: () => { },
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['BillReports']);
