const App = {
    setup() {
        ...
        const services = {
            getPDFData: async (id) => {
                try {
                    const response = await AxiosManager.get('/DebitNote/GetDebitNoteSingle?id=' + id, {});
                    return response;
                } catch (error) {
                    throw error;
                }
        ...
        const methods = {
            populatePDFData: async (id) => {
                ...
                state.purchaseReturnNumber = pdfData?.purchaseReturn?.number;
                state.items = response?.data?.content?.transactionList?.map(item => ({
                    ...item,
                    total: item.product.unitPrice * item.movement
                })) || [];
                state.vendor = pdfData?.purchaseReturn?.goodsReceive?.purchaseOrder?.vendor || {};
                state.debitNoteNumber = pdfData.number || '';
                state.debitNoteDate = DateFormatManager.formatToLocale(pdfData.debitNoteDate) || '';
                state.debitNoteCurrency = StorageManager.getCompany()?.currency || '';
            ...
            downloadPDF: async () => {
                ...
                try {
                    ...
                    await html2canvas(content, {
                        ...
                        doc.save(`debit-note-${state.debitNoteNumber || 'unknown'}.pdf`);
                    });
                } catch (error) {
