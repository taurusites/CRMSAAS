    ...
    setup() {
        ...
        const services = {
            ...
            createMainData: async (name, description, createdById) => {
                try {
                    const response = await AxiosManager.post('/BookingGroup/CreateBookingGroup', {
            ...
            updateMainData: async (id, name, description, updatedById) => {
                try {
                    const response = await AxiosManager.post('/BookingGroup/UpdateBookingGroup', {
            ...
            deleteMainData: async (id, deletedById) => {
                try {
                    const response = await AxiosManager.post('/BookingGroup/DeleteBookingGroup', {
                        id, deletedById
                    });
                    return response;
                } catch (error) {
            ...
            handleSubmit: async function () {
                try {
                    ...
                    if (response.data.code === 200) {
                        ...
                        if (!state.deleteMode) {
                            state.mainTitle = 'Edit Booking Group';
        ...
        Vue.onMounted(async () => {
            try {
                await SecurityManager.authorizePage(['BookingGroups']);
                ...
                mainGrid.obj = new ej.grids.Grid({
                    ...
                    toolbarClick: async (args) => {
                        ...
                        if (args.item.id === 'AddCustom') {
                            ...
                            state.mainTitle = 'Add Booking Group';
                            resetFormState();
                            mainModal.obj.show();
                        }
                        ...
                        if (args.item.id === 'DeleteCustom') {
                            ...
                            if (mainGrid.obj.getSelectedRecords().length) {
                                const selectedRecord = mainGrid.obj.getSelectedRecords()[0];
                                state.mainTitle = 'Delete Booking Group?';
                                state.id = selectedRecord.id ?? '';
                                state.name = selectedRecord.name ?? '';
                                state.description = selectedRecord.description ?? '';
                                mainModal.obj.show();
