const App = {
    setup() {
        ...
        const services = {
            getPDFData: async (id) => {
                try {
                    const response = await AxiosManager.get('/SalesQuotation/GetSalesQuotationSingle?id=' + id, {});
                    return response;
                } catch (error) {
                    throw error;
                }
        ...
        const methods = {
            populatePDFData: async (id) => {
                ...
                state.items = pdfData.salesQuotationItemList || [];
                state.customer = pdfData.customer || {};
                state.quotationNumber = pdfData.number || '';
                state.quotationDate = DateFormatManager.formatToLocale(pdfData.quotationDate) || '';
                state.quotationCurrency = StorageManager.getCompany()?.currency || '';
            ...
            downloadPDF: async () => {
                ...
                try {
                    ...
                    await html2canvas(content, {
                        ...
                        doc.save(`sales-quotation-${state.quotationNumber || 'unknown'}.pdf`);
                    });
                } catch (error) {
